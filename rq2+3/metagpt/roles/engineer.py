...
class Engineer(Role):
    ...
    def __init__(
        ...
        personality:str ="", # [TOMOIKE EDIT]
    ) -> None:
        """Initializes the Engineer role with given attributes."""
        super().__init__(name, profile, goal, constraints, personality=personality)
    ...
    async def _act_mp(self) -> Message:
        ...
        for todo in self.todos:
            todo_coro = WriteCode(personality=self.personality).run(
    ...
    async def _act_sp(self) -> Message:
        ...
        for todo in self.todos:
            code = await WriteCode(personality=self.personality).run(context=self._rc.history, filename=todo)
            # logger.info(todo)
            # logger.info(code_rsp)
    ...
    async def _act_sp_precision(self) -> Message:
        ...
        for todo in self.todos:
            ...
            context_str = "\n".join(context)
            # Write code
            code = await WriteCode(personality=self.personality).run(context=context_str, filename=todo)
            # Code review
            if self.use_code_review:
                try:
                    rewrite_code = await WriteCodeReview(personality=self.personality).run(context=context_str, code=code, filename=todo)
                    code = rewrite_code
